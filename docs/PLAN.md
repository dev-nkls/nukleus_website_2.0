...
| Link-preview / home-screen icon strategy | Single light OG image; PNG home-screen icons generated at build time | 2026-05-01 | `opengraph-image.tsx` (1200×630) ships ONE image — link previews don't theme-switch, and a light image reads cleanly on both light and dark host surfaces. `apple-icon.tsx` (180×180) for iOS, `icon1.tsx` (512×512) + `manifest.ts` for Android. Generated via `next/og` `ImageResponse` so the brand can be tweaked in code; no separate design files to keep in sync. |
...
- [x] Link previews + home-screen icons (2026-05-01): `opengraph-image.tsx` generates a 1200×630 PNG at build time so iMessage / Slack / Twitter / LinkedIn show the brand-mark card on share. `apple-icon.tsx` (180×180) covers iOS pinned-site, `icon1.tsx` (512×512) + `manifest.ts` cover Android Add-to-Home-Screen + PWA install metadata. All four routes static-prerendered.
