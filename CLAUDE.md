...
src/
  app/
    ...
    opengraph-image.tsx       # auto-generated 1200×630 PNG → /opengraph-image, wires <meta og:image> + <meta twitter:image>. Cream surface, brand mark + NUKLEUS lockup left, tagline right.
    apple-icon.tsx            # auto-generated 180×180 PNG → /apple-icon, wires <link rel="apple-touch-icon"> for iOS pinned-site / Add-to-Home-Screen.
    icon1.tsx                 # auto-generated 512×512 PNG → /icon1, second <link rel="icon"> sibling of icon.svg, used by Android Add-to-Home-Screen + PWA install.
    manifest.ts               # /manifest.webmanifest — name, theme_color #a01818, background_color #f8f8fc, icons[] = icon.svg + icon1 + apple-icon. Drives Android home-screen + PWA install metadata.
...
- **iOS / Android home-screen icons:** generated by `apple-icon.tsx` (180×180) and `icon1.tsx` (512×512) at build time using `next/og` `ImageResponse`. Both render the brand mark on an opaque cream surface. Apple specifically requires opaque backgrounds; Android prefers PNG over SVG for home-screen reliability.
- **Open Graph / Twitter Card image:** generated by `opengraph-image.tsx` (1200×630, ~110KB). Cream surface mirroring the particle-aligned reference SVG: brand mark + NUKLEUS wordmark + red accent line on the left, tagline + sub-line on the right. Auto-emitted as `<meta property="og:image">` and `<meta name="twitter:image">`. Single image (no light/dark variants — link previews don't theme-switch). To bust caches after a redesign: Facebook debugger (https://developers.facebook.com/tools/debug/) → Scrape Again; LinkedIn post-inspector; Slack/iMessage caches expire on their own (~1h / ~24h respectively).
...
- **Approach page family:**
  ...
  - `.approach-pillars` + `.approach-pillar` + `.pillar-desc` — flat cards. On desktop: always-visible numeral + title + description with hover fill. On mobile (≤720px): collapsed to a single inline row (`grid-template-columns: auto 1fr`) so the numeral and title share a baseline; description spans the full row when expanded. Tap focuses the card. Light mode: focus floods brand-red with white text. Dark mode (`[data-theme="dark-red"]`): rest + focus both stay glassy black (`rgba(0,0,0,0.55)` / `rgba(0,0,0,0.62)` + 14px backdrop-blur), numeral keeps its red stroke so the brand colour still reads.
  ...
  - `.timeline` + `.timeline-step` — vertical timeline. Inside `.approach-engagement`, each step is a translucent white card in light mode and a glassy-black card in dark mode (`rgba(0,0,0,0.55)` + 14px blur, faint white border). On mobile, collapsed to marker + title; tap flips the card red (light) or stays glassy black (dark) with white text and reveals the description.
- `.about-prose` + `.about-prose-panel` — single-column prose with red Fraunces drop cap on `p.first`. The panel adds a tight translucent backdrop so the copy reads against the page-bg video. Dark mode flips the wash from white-tinted (`rgba(255,255,255,0.16)`) to black-tinted (`rgba(0,0,0,0.45)` + 10px blur + faint white border) so it reads as glassy black instead of milky white.
- `.lede-panel` — tight translucent inline-block panel applied to lede paragraphs on services and approach so they read clearly against the video without any visible card edge. Same dark-mode treatment as `.about-prose-panel`.
